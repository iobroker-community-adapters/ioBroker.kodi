...
let timer;
let reconnectTimer;
let getPlayListTimer;
let SwitchPVRTimer;
let GetSourcesTimer;
let GetNameVersionTimer;
let infoFileTimer;
let version;
...
let states = {
    ...
    main: {
        ...
        repeat: {
            ...
            read: true,
            write: true,
            states: {off: 'Off', all: 'All', one: 'One'}
        },
        shuffle: {
            val: false,
        ...
        setsubtitle: {
            ...
            read: false,
            write: true,
            states: {previous: 'Previous', next: 'Next', off: 'Off', on: 'On'}
        },
...
function clearInfo() {
    ...
    for (let key in states.info) {
        if (!Object.hasOwnProperty.call(states.info, key)) {
            continue;
        }
...
function ShowNotification(param, callback) {
    ...
    callback({title, message, image, displaytime});
}
